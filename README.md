...
Large Telegram files use two parallel aligned download lanes by default (`TELEGRAM_DOWNLOAD_CONNECTIONS=2`) once they reach `PARALLEL_DOWNLOAD_MIN_SIZE_MB=64`. Set the connection count to `1` to use Telethon's standard sequential downloader. More connections are not always faster and may worsen an unstable media-DC route; increase gradually and do not exceed the validated maximum of 16.
